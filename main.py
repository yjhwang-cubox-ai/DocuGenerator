...
def run(args):
    if args.config is not None:
        config = synthdocs.read_config(args.config)

    pprint.pprint(config)

    synthdocs.set_global_random_seed(args.seed)
    template = synthdocs.read_template(args.script, args.name, config)
    generator = synthdocs.generator(
        args.script,
        args.name,
        config=config,
        count=args.count,
